...
const brands = require('../resources/microwave-brands');
...
class StatePickRefrigeratorBrands extends StateBase {
  ...
  constructReply() {
    const self = this;
    let message = {
      text: self.localesManager.questions.pickMicrowaveBrand,
      quick_replies: []
    };
  ...
  verifyInput(callback) {
    console.log('Verifying Inputs in state pick a microwave brand');
    ...
    let microwavesFilters = user.filters;
    ...
    microwavesFilters.brand = pickedBrand;
    // testing only
